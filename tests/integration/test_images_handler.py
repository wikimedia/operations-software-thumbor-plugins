...
class WikimediaImagesHandlerTestCase(WikimediaTestCase):
    ...
    def get_config(self):
        ...
        cfg.COMMUNITY_EXTENSIONS = ["wikimedia_thumbor.handler.images", "wikimedia_thumbor.handler.core"]
        ...
        cfg.SWIFT_KEY = "baz"
        cfg.SWIFT_SHARDED_CONTAINERS = ["wikipedia-en-local-public", "wikipedia-en-local-temp", "wikipedia-en-local-thumb", "wikipedia-commons-local-public", "wikipedia-commons-local-thumb"]
    ...
    def run_and_check_headers(
        ...
        try:
            wikimedia_original_container = response.headers.get_list("Thumbor-Wikimedia-Original-Container")[0]
        except IndexError:
            wikimedia_original_container = None

        try:
            wikimedia_original_path = response.headers.get_list("Thumbor-Wikimedia-Original-Path")[0]
        except IndexError:
            wikimedia_original_path = None

        try:
            wikimedia_thumbnail_container = response.headers.get_list("Thumbor-Wikimedia-Thumbnail-Container")[0]
        except IndexError:
            wikimedia_thumbnail_container = None

        try:
            wikimedia_thumbnail_path = response.headers.get_list("Thumbor-Wikimedia-Thumbnail-Path")[0]
        except IndexError:
            wikimedia_thumbnail_path = None

        try:
            thumbor_parameters = json.loads(response.headers.get_list("Thumbor-Parameters")[0])
        except IndexError:
            thumbor_parameters = {"width": None, "image": None, "filters": None}

        try:
            wikimedia_content_diposition = response.headers.get_list("Content-Disposition")[0]
        except IndexError:
            wikimedia_content_diposition = None

        assert xkey == expected_xkey, f"Incorrect Xkey: {xkey}"

        assert wikimedia_original_container == expected_original_container, f"Thumbor-Wikimedia-Original-Container: {wikimedia_original_container}"

        assert wikimedia_original_path == expected_original_path, f"Thumbor-Wikimedia-Original-Path: {wikimedia_original_path}"

        assert wikimedia_thumbnail_container == expected_thumbnail_container, f"Thumbor-Wikimedia-Thumbnail-Container: {wikimedia_thumbnail_container}"

        assert wikimedia_thumbnail_path == expected_thumbnail_path, f"Thumbor-Wikimedia-Thumbnail-Path: {wikimedia_thumbnail_path}"

        assert thumbor_parameters["width"] == expected_width, "Thumbor-Parameters width: {}".format(thumbor_parameters["width"])

        assert thumbor_parameters["image"] == expected_image, "Thumbor-Parameters image: {}".format(thumbor_parameters["image"])

        assert thumbor_parameters["filters"] == expected_filters, "Thumbor-Parameters filters: {}".format(thumbor_parameters["filters"])

        assert wikimedia_content_diposition == expected_content_disposition, f"Content-Disposition: {wikimedia_content_diposition}"
    ...
    def test_broken_request(self):
        self.run_and_check_headers(
            ...
            None,
            "400",
            "http://swifthost/swift/v1/api/path/" + "wikipedia-en-local-public.d3/d/d3/1M\ncolors.tif",
            "format(png):page(1)",
    ...
    def run_and_check_error(self, url, expected_status):
        response = self.fetch(url)
        assert response.code == expected_status

    def test_missing_mandatory_parameters(self):
        self.run_and_check_error("/wikipedia/en/thumb/d/d3/1Mcolors.tif", 404)

    def test_too_many_parts(self):
        self.run_and_check_error("/wikipedia/en/thumb/8/80/1Mcolors.tif/1Mcolors.tif.jpg/90x55x2-15px-1Mcolors.tif.jpg", 404)
        self.run_and_check_error("/wikipedia/en/thumb/7/77/1Mcolors.png/1Mcolors.png/199px-1Mcolors.png", 404)
