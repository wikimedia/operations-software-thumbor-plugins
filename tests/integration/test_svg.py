...
class WikimediaSvgTest(WikimediaTestCase):
    ...
    def test_lang_variant_not_stripped(self):
        ...
        with patch.object(ShellRunner, "popen", record_popen):
            result = self.fetch("/thumbor/unsafe/200x/filters:lang(sr-latn):format(png)/Speech_bubbles.svg")

        assert result.code == 200, f"Response code: {result.code}"

        rsvg_commands = [command for command in commands if os.path.basename(command[0]) == "rsvg-convert"]

        assert rsvg_commands, f"rsvg-convert was never called: {commands!r}"

        for command in rsvg_commands:
            assert "--accept-language" in command, f"No language passed to rsvg-convert: {command!r}"

            passed_lang = command[command.index("--accept-language") + 1]

            assert passed_lang == "sr-latn", f"Language tag altered: {passed_lang} (should be sr-latn)"
