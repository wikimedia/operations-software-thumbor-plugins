...
class WikimediaVipsTest(WikimediaTestCase):
    ...
    def inject_fixtures(self, caplog, monkeypatch):
        self.caplog = caplog
        monkeypatch.setattr("tests.integration.which", lambda cmd: "wrong/path" if cmd == "vips" else which(cmd))
    ...
    def test_skip_factor_1(self):
        self.run_and_check_ssim_and_size(
            url=("/thumbor/unsafe/2000x/filters:format(png)/Lakedaimoniergrab_Zeichnung_und_Steinplan.png"),
            mediawiki_reference_thumbnail=("2000px-Lakedaimoniergrab_Zeichnung_und_Steinplan.png"),
            perfect_reference_thumbnail=("2000px-Lakedaimoniergrab_Zeichnung_und_Steinplan.png"),
            expected_width=2000,
            expected_height=987,
            expected_ssim=0.99,
            size_tolerance=1.01,
        ...
        self.run_and_check_ssim_and_size(
            url=("/thumbor/unsafe/2000x/filters:format(webp)/Lakedaimoniergrab_Zeichnung_und_Steinplan.png"),
            mediawiki_reference_thumbnail=("2000px-Lakedaimoniergrab_Zeichnung_und_Steinplan.png"),
            perfect_reference_thumbnail=("2000px-Lakedaimoniergrab_Zeichnung_und_Steinplan.png"),
            expected_width=2000,
            expected_height=987,
