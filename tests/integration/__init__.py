...
class WikimediaTestCase(AsyncHTTPTestCase):
    def get_config(self):
        ...
        cfg.FILE_LOADER_ROOT_PATH = os.path.join(os.path.dirname(__file__), "originals")
        ...
        timeout = which("gtimeout" if platform.system() == "Darwin" else "timeout")
        ...
        cfg.EXIF_TINYRGB_PATH = os.path.realpath(os.path.join(os.path.dirname(__file__), "..", "..", "tinyrgb.icc"))
    ...
    def run_and_check_ssim_and_size(self, url, mediawiki_reference_thumbnail, perfect_reference_thumbnail, expected_width, expected_height, expected_ssim, size_tolerance, headers=None):
        ...
        expected_path = os.path.join(os.path.dirname(__file__), "thumbnails", mediawiki_reference_thumbnail)

        visual_expected_path = os.path.join(os.path.dirname(__file__), "thumbnails", perfect_reference_thumbnail)
        visual_expected = Image.open(visual_expected_path).convert(generated.mode)

        assert generated.size[0] == expected_width, "Width differs: %d (should be == %d)\n" % (generated.size[0], expected_width)

        assert generated.size[1] == expected_height, "Height differs: %d (should be == %d)\n" % (generated.size[1], expected_height)
        ...
        assert ratio <= size_tolerance, f"Generated file bigger than size tolerance: {ratio:f} (should be <= {size_tolerance:f})"
