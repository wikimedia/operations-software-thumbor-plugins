...
class WikimediaSwiftTestCase(WikimediaTestCase):
    def setUp(self):
        ...
        def fetch_impl(self, request, callback):
            expected = "http://swifthost/swift/v1/api/path/" + "wikipedia-en-local-public/d/d3/1Mcolors.png"
    ...
    def get_config(self):
        ...
        cfg.COMMUNITY_EXTENSIONS = ["wikimedia_thumbor.handler.images"]
        ...
        cfg.SWIFT_KEY = "baz"
        cfg.SWIFT_SHARDED_CONTAINERS = ["wikipedia-en-local-public", "wikipedia-en-local-thumb"]
    ...
    def mock_put_object(self, container, obj, contents, content_length=None, etag=None, chunk_size=None, content_type=None, headers=None, query_string=None, response_dict=None):
    ...
    def mock_get_object(self, container, obj, resp_chunk_size=None, query_string=None, response_dict=None, headers=None):
        ...
        if self.get_object_calls == 1:
            ...
            path = os.path.join(os.path.dirname(__file__), "thumbnails", "400px-1Mcolors.png")
            with open(path, "rb") as f:
                return {}, f.read()
