...
class WikimediaExifTest(WikimediaTestCase):
    ...
    def inject_fixtures(self, caplog, monkeypatch):
        self.caplog = caplog
        monkeypatch.setattr("tests.integration.which", lambda cmd: "wrong/path" if cmd == "exiftool" else which(cmd))

    def run_and_check_exif(self, url, expected_exif_fields, expected_icc_profile):
        ...
        if expected_icc_profile:
            assert result_image.info["icc_profile"] == expected_icc_profile, "ICC profile: {}".format(result_image.info["icc_profile"])
    ...
    def check_exif(self, result_buffer, expected):
        ...
        stdout = ExiftoolRunner().command(pre=exiftool_args, context=self.ctx, input_temp_file=temp_file)
    ...
    def test_adobe_rgb(self):
        ...
        self.run_and_check_exif("/thumbor/unsafe/300x/filters:conditional_sharpen(0.0,0.8,1.0,0.0,0.85)/Physical_map_tagged_AdobeRGB.jpg", None, adobe_rgb)
        self.run_and_check_exif("/thumbor/unsafe/300x/filters:conditional_sharpen(0.0,0.8,1.0,0.0,0.85):format(webp)/Physical_map_tagged_AdobeRGB.jpg", None, adobe_rgb)
    ...
    def test_tinyrgb_substitution(self):
        tinyrgb_path = os.path.realpath(os.path.join(os.path.dirname(__file__), "..", "..", "tinyrgb.icc"))
        with open(tinyrgb_path, "rb") as tinyrgb_file:
            tinyrgb = tinyrgb_file.read()

        self.run_and_check_exif("/thumbor/unsafe/400x/filters:conditional_sharpen(0.0,0.8,1.0,0.0,0.85)/Christophe_Henner_-_June_2016.JPG", None, tinyrgb)
        self.run_and_check_exif("/thumbor/unsafe/400x/filters:conditional_sharpen(0.0,0.8,1.0,0.0,0.85):format(webp)/Christophe_Henner_-_June_2016.JPG", None, tinyrgb)
