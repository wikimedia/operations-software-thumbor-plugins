...
def _mc(self):
    ...
    if hasattr(self, "failure_mc"):
        ...
        if dead_servers and len(dead_servers) == len(servers):
            logger.error("[MEMCACHED] Returning client object when all servers are marked dead", extra=log_extra(self.context))
...
class ImagesHandler(ImagingHandler):
    ...
    def generate_save_swift_path(self, kw):
        path = "/".join((kw["shard1"], kw["shard2"], kw["filename"] + "." + kw["extension"], ""))
    ...
    def translate(self, kw):
        ...
        original_filepath = "/".join((original_shard1, original_shard2, kw["filename"] + "." + kw["extension"]))
        ...
        if kw["specialpath"] and kw["specialpath"] != "temp":
            original_filepath = "/".join((kw["specialpath"], original_filepath))

        swift_original_uri = self.context.config.SWIFT_HOST + self.context.config.SWIFT_API_PATH + "/" + original_container
        ...
        if kw["extension"] == "svg":
            if hasattr(self.context.config, "SVG_MAX_SIZE") and (self.request.width > self.context.SVG_MAX_SIZE or self.request.height > self.config.SVG_MAX_SIZE):
                self._error(422, f"Requested size {self.context.request.width} is larger than maximum ({self.context.config.SVG_MAX_SIZE})")

        if kw["extension"].lower() in ("jpg", "jpe", "jpeg"):
            if kw["qlow"] == "qlow-" and hasattr(self.context.config, "QUALITY_LOW"):
        ...
        self.context.wikimedia_thumbnail_save_path = self.generate_save_swift_path(kw)

        if hasattr(self.context.config, "SWIFT_PATH_PREFIX"):
            self.context.wikimedia_thumbnail_save_path = self.context.config.SWIFT_PATH_PREFIX + self.context.wikimedia_thumbnail_save_path
    ...
    def set_headers(self, translated):
        ...
        self.safe_set_header("Content-Disposition", "inline;filename*=UTF-8''{}".format(quote(content_disposition.encode("utf-8"))))

        self.safe_set_header("Thumbor-Wikimedia-Original-Container", self.context.wikimedia_original_container)

        self.safe_set_header("Thumbor-Wikimedia-Thumbnail-Container", self.context.wikimedia_thumbnail_container)

        self.safe_set_header("Thumbor-Wikimedia-Original-Path", self.context.wikimedia_original_filepath.encode("utf-8"))

        self.safe_set_header("Thumbor-Wikimedia-Thumbnail-Path", self.context.wikimedia_thumbnail_save_path.encode("utf-8"))

        self.safe_set_header("Thumbor-Parameters", json.dumps(translated))

        self.safe_set_header("Proxy-Request-Date", self.request.headers.get("Proxy-Request-Date", "None"))
    ...
    async def check_image(self, kw):
        ...
        self.safe_set_header("Thumbor-Request-Date", format_date_time(timestamp))
        ...
        if self.context.private:
            ...
            if not secret or not received_secret or received_secret != secret:
                self._error(401, "Unauthorized access to private Swift container")
                return
        ...
        if mc and xkey:
            ...
            start = datetime.datetime.now()
            counter = _counter_value(await tornado.ioloop.IOLoop.instance().run_in_executor(None, mc.get, key))
            ...
            if counter and counter >= self.context.config.get("FAILURE_THROTTLING_MAX", 4):
                ...
                self._error(429, "Too many thumbnail requests for failing image")
                return
    ...
    async def poolcounter_throttle_key(self, key, cfg):
        ...
        self._error(429, "Too many thumbnail requests")
        return True
    ...
    async def poolcounter_throttle(self, filename, extension):
        ...
        if release_timeout:
            ...
            tornado.ioloop.IOLoop.instance().call_later(release_timeout, partial(close_poolcounter, self.pc))
