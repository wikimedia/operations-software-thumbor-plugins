...
class Engine(BaseWikimediaEngine):
    def should_run(self, buffer):
        ...
        if hasattr(self.context, "wikimedia_original_file"):
            stdout = Engine.exiftool.command(pre=command, context=self.context, input_temp_file=self.context.wikimedia_original_file)
        else:
            stdout = Engine.exiftool.command(pre=command, context=self.context, buffer=buffer)
    ...
    def create_image(self, buffer):
        ...
        shrink_factor = max(1, int(self.context.vips["width"] // (2 * self.context.request.width)))
    ...
    def shrink(self, buffer, shrink_factor):
        ...
        try:
            source = "%s[page=%d]" % (self.source, self.context.request.page - 1)
    ...
    def shrink_for_page(self, source, shrink_factor, output_args=""):
        temp_dir = mkdtemp()
        destination = os.path.join(temp_dir, "vips_result.png")

        command = [self.context.config.VIPS_PATH, "shrink", source, destination + output_args, "%d" % shrink_factor, "%d" % shrink_factor]
